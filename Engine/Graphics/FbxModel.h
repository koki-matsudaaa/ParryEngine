...
// ──────────────────────────────────────────
// FBXモデル (第1段階: 頂点位置だけ・単色表示)
//   PmdModel と同じく IRenderable を継承するので、RenderComponent に
//   そのまま差せて、既存の描画ループに乗る。
//   法線・UV・テクスチャ・マテリアルは第2段階で追加する。
// ──────────────────────────────────────────
...
public:
    ...
    // 今のフレームのボーン行列一式を返す (描画側が定数バッファに書くため)。
...
private:
    ...
    // マテリアルからテクスチャのパスを出す
    std::string ExtractTexturePath(FbxScene* scene);
};